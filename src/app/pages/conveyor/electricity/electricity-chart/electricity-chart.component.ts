...
export class ElectricityChartComponent implements AfterViewInit, OnDestroy, OnChanges {
      ...
      .subscribe(config => {
        ...
        this.option = {
          ...
          xAxis: {
            ...
            boundaryGap: false,
            offset: 25,
            data: this.prepAxisXLabels(this.data.axisXLabels),
            axisTick: {
              show: false,
            },
            axisLabel: {
  ...
  updateOrdersChartOptions(data: ChartData) {
    const options = this.option;
    const series = this.getNewSeries(options.series, data.linesData);
    const xAxis = this.getNewXAxis(options.xAxis, this.prepAxisXLabels(data.axisXLabels));

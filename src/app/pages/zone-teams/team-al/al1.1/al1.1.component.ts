...
@Component({
  selector: 'ngx-al1_1',
  templateUrl: './al1.1.component.html',
  styleUrls: ['./al1.1.component.scss']
})
export class Al1_1Component implements OnInit {

  private alive=true;

  public TeamAL1_1: DeviceXpl;

  
  public velocidadAL1_1: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeAL1_1')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadAL1_1;
  ...
  ngAfterViewInit(): void {
    ...
    this.tooltipInterval1 = window.setInterval(
        ...
        (): void => {
            if (document.getElementById('sample-dataAL1_1')) {
                let value1: number = this.velocidadAL1_1;
...
changeAL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL1_1')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamAL1_1 = res[0]
      this.velocidadAL1_1 = this.TeamAL1_1?.VelocidadActualMotor ?? 0;
  ...
  dataAL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL1_1'))
    )
    .subscribe((res: any) => {

...
@Component({
  selector: 'ngx-al1_8',
  templateUrl: './al1.8.component.html',
  styleUrls: ['./al1.8.component.scss']
})
export class Al1_8Component implements OnInit {

  private alive=true;

  public TeamAL1_8: DeviceXpl;

  
  public velocidadAL1_8: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeAL1_8')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadAL1_8;
  ...
  ngAfterViewInit(): void {
    ...
    this.tooltipInterval1 = window.setInterval(
        ...
        (): void => {
            if (document.getElementById('sample-dataAL1_8')) {
                let value1: number = this.velocidadAL1_8;
...
changeAL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL1_8')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamAL1_8 = res[0]
      this.velocidadAL1_8 = this.TeamAL1_8?.VelocidadActualMotor ?? 0;
  ...
  dataAL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL1_8'))
    )
    .subscribe((res: any) => {

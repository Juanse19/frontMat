...
@Component({
  selector: 'ngx-al2_1',
  templateUrl: './al2.1.component.html',
  styleUrls: ['./al2.1.component.scss']
})
export class Al2_1Component implements OnInit {

  private alive=true;

  public TeamAL2_1: DeviceXpl;

  
  public velocidadAL2_1: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeAL2_1')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadAL2_1;
  ...
  ngAfterViewInit(): void {
    ...
    this.tooltipInterval1 = window.setInterval(
        ...
        (): void => {
            if (document.getElementById('sample-dataAL2_1')) {
                let value1: number = this.velocidadAL2_1;
...
changeAL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_1')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamAL2_1 = res[0]
      this.velocidadAL2_1 = this.TeamAL2_1?.VelocidadActualMotor ?? 0;
  ...
  dataAL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_1'))
    )
    .subscribe((res: any) => {

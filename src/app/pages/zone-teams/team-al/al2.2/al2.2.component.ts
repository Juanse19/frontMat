...
@Component({
  selector: 'ngx-al2_2',
  templateUrl: './al2.2.component.html',
  styleUrls: ['./al2.2.component.scss']
})
export class Al2_2Component implements OnInit {

  private alive=true;

  public TeamAL2_2: DeviceXpl;

  
  public velocidadAL2_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeAL2_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadAL2_2;
  ...
  ngAfterViewInit(): void {
    ...
    this.tooltipInterval1 = window.setInterval(
        ...
        (): void => {
            if (document.getElementById('sample-dataAL2_2')) {
                let value1: number = this.velocidadAL2_2;
...
changeAL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamAL2_2 = res[0]
      this.velocidadAL2_2 = this.TeamAL2_2?.VelocidadActualMotor ?? 0;
  ...
  dataAL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_2'))
    )
    .subscribe((res: any) => {

...
@Component({
  selector: 'ngx-al2_4',
  templateUrl: './al2.4.component.html',
  styleUrls: ['./al2.4.component.scss']
})
export class Al2_4Component implements OnInit {

  private alive=true;

  public TeamAL2_4: DeviceXpl;

  
  public velocidadAL2_4: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeAL2_4')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadAL2_4;
  ...
  ngAfterViewInit(): void {
    ...
    this.tooltipInterval1 = window.setInterval(
        ...
        (): void => {
            if (document.getElementById('sample-dataAL2_4')) {
                let value1: number = this.velocidadAL2_4;
...
changeAL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_4')
    ...
    .subscribe((res: any)=>{
      if(res == timeout(100)){
        ...
        console.log('No hay data AL1_1');
        res.VelocidadActualMotor = 0;
        res.CorrienteActualMotor = 0;
        res.PotenciaActualMotor = 0;
        res.KWh = 0;
      } else {
      this.TeamAL2_4 = res[0]
      this.velocidadAL2_4 = this.TeamAL2_4?.VelocidadActualMotor ?? 0;
      // console.group('Gauge');
      // console.log('Prueba ', this.TeamAL2_4);
      // console.groupEnd()
    }
    this.dataAL1_1();
    });
  ...
  dataAL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'AL2_4'))
    )
    .subscribe((res: any) => {

...
export interface CL1_1 {
  CL1_1_VelocidadActualMotor: number,
  CL1_1_CorrienteActualMotor: number,
  CL1_1_PotenciaActualMotor: number,
  CL1_1_TorqueActualMotor: number,
  CL1_1_KWh: number,
  CL1_1_voltaje: number
}
...
export class Cl1_1Component implements OnInit {

  private alive=true;

  public TeamCL1_1: DeviceXpl;

  public velocidadCL1_1: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_1')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_1;
  ...
  changeCL1_1() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_1')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_1 = this.TeamCL1_1?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_1();
    });
  }
  ...
  dataCL1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_1'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_1');

...
export interface CL1_10 {
  CL1_10_VelocidadActualMotor: number,
  CL1_10_CorrienteActualMotor: number,
  CL1_10_PotenciaActualMotor: number,
  CL1_10_TorqueActualMotor: number,
  CL1_10_KWh: number,
  CL1_10_voltaje: number
}
...
export class Cl1_10Component implements OnInit {

  private alive=true;

  public TeamCL1_10: DeviceXpl;

  public velocidadCL1_10: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_10')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_10;
  ...
  changeCL1_10() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_10')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_10 = this.TeamCL1_10?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_10();
    });
  }
  ...
  dataCL1_10(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_10'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_10');

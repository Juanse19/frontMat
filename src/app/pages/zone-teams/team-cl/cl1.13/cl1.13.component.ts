...
export interface CL1_13 {
  CL1_13_VelocidadActualMotor: number,
  CL1_13_CorrienteActualMotor: number,
  CL1_13_PotenciaActualMotor: number,
  CL1_13_TorqueActualMotor: number,
  CL1_13_KWh: number,
  CL1_13_voltaje: number
}
...
export class Cl1_13Component implements OnInit {

  private alive=true;

  public TeamCL1_13: DeviceXpl;

  public velocidadCL1_13: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_13')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_13;
  ...
  changeCL1_13() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_13')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_13 = this.TeamCL1_13?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_13();
    });
  }
  ...
  dataCL1_13(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_13'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_13');

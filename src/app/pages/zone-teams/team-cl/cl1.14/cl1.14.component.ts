...
export interface CL1_14 {
  CL1_14_VelocidadActualMotor: number,
  CL1_14_CorrienteActualMotor: number,
  CL1_14_PotenciaActualMotor: number,
  CL1_14_TorqueActualMotor: number,
  CL1_14_KWh: number,
  CL1_14_voltaje: number
}
...
export class Cl1_14Component implements OnInit {

  private alive=true;

  public TeamCL1_14: DeviceXpl;

  public velocidadCL1_14: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_14')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_14;
  ...
  changeCL1_14() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_14')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_14 = this.TeamCL1_14?.VelocidadActualMotor ?? 0;
    }
    });
  }
  ...
  dataCL1_14(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_14'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_14');

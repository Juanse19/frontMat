...
export interface CL1_15 {
  CL1_15_VelocidadActualMotor: number,
  CL1_15_CorrienteActualMotor: number,
  CL1_15_PotenciaActualMotor: number,
  CL1_15_TorqueActualMotor: number,
  CL1_15_KWh: number,
  CL1_15_voltaje: number
}
...
export class Cl1_15Component implements OnInit {

  private alive=true;

  public TeamCL1_15: DeviceXpl;

  public velocidadCL1_15: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_15')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_15;
  ...
  changeCL1_15() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_15')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_15 = this.TeamCL1_15?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_15();
    });
  }
  ...
  dataCL1_15(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_15'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_15');

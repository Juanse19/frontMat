...
public tooltipInterval1: number;
  ...
  changeCL1_16() {
    this.apiGetComp.GetJson(this.api.apiUrlNode1 + '/api/v1/device?device=' + 'CL1_16')
  ...
  dataCL1_16(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.apiUrlNode1 + '/api/v1/device?device=' + 'CL1_16'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_16');

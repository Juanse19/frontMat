...
public tooltipInterval1: number;
  ...
  changeCL1_16() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_16')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_16 = this.TeamCL1_16?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_16();
    });
  }
  ...
  dataCL1_16(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_16'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_16');

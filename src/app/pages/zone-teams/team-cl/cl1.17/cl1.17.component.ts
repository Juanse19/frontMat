...
export interface CL1_17 {
  CL1_17_VelocidadActualMotor: number,
  CL1_17_CorrienteActualMotor: number,
  CL1_17_PotenciaActualMotor: number,
  CL1_17_TorqueActualMotor: number,
  CL1_17_KWh: number,
  CL1_17_voltaje: number
}
...
export class Cl1_17Component implements OnInit {

  private alive=true;

  public TeamCL1_17: DeviceXpl;

  public velocidadCL1_17: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_17')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_17;
  ...
  changeCL1_17() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_17')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_17 = this.TeamCL1_17?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_17();
    });
  }
  ...
  dataCL1_17(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_17'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_17');

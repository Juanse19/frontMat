...
export interface CL1_18 {
  CL1_18_VelocidadActualMotor: number,
  CL1_18_CorrienteActualMotor: number,
  CL1_18_PotenciaActualMotor: number,
  CL1_18_TorqueActualMotor: number,
  CL1_18_KWh: number,
  CL1_18_voltaje: number
}
...
export class Cl1_18Component implements OnInit {

  private alive=true;

  public TeamCL1_18: DeviceXpl;

  public velocidadCL1_18: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_18')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_18;
  ...
  changeCL1_18() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_18')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_18 = this.TeamCL1_18?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_18();
    });
  }
  ...
  dataCL1_18(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_18'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_18');

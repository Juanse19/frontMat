...
export interface CL1_2 {
  CL1_2_VelocidadActualMotor: number,
  CL1_2_CorrienteActualMotor: number,
  CL1_2_PotenciaActualMotor: number,
  CL1_2_TorqueActualMotor: number,
  CL1_2_KWh: number,
  CL1_2_voltaje: number
}
...
export class Cl1_2Component implements OnInit {

  private alive=true;

  public TeamCL1_2: DeviceXpl;

  public velocidadCL1_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_2;
  ...
  changeCL1_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_2 = this.TeamCL1_2?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_2();
    });
  }
  ...
  dataCL1_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_2'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_2');

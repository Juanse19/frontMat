...
export interface CL1_3 {
  CL1_3_VelocidadActualMotor: number,
  CL1_3_CorrienteActualMotor: number,
  CL1_3_PotenciaActualMotor: number,
  CL1_3_TorqueActualMotor: number,
  CL1_3_KWh: number,
  CL1_3_voltaje: number
}
...
export class Cl1_3Component implements OnInit {

  private alive=true;

  public TeamCL1_3: DeviceXpl;

  public velocidadCL1_3: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_3')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_3;
  ...
  changeCL1_3() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_3')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_3 = this.TeamCL1_3?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_3();
    });
  }
  ...
  dataCL1_3(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_3'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_3');

...
export interface CL1_4 {
  CL1_4_VelocidadActualMotor: number,
  CL1_4_CorrienteActualMotor: number,
  CL1_4_PotenciaActualMotor: number,
  CL1_4_TorqueActualMotor: number,
  CL1_4_KWh: number,
  CL1_4_voltaje: number
}
...
export class Cl1_4Component implements OnInit {

  private alive=true;

  public TeamCL1_4: DeviceXpl;

  public velocidadCL1_4: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_4')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_4;
  ...
  changeCL1_4() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_4')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_4 = this.TeamCL1_4?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_4();
    });
  }
  ...
  dataCL1_4(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_4'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_4');

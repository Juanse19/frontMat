...
export interface CL1_5 {
  CL1_5_VelocidadActualMotor: number,
  CL1_5_CorrienteActualMotor: number,
  CL1_5_PotenciaActualMotor: number,
  CL1_5_TorqueActualMotor: number,
  CL1_5_KWh: number,
  CL1_5_voltaje: number
}
...
export class Cl1_5Component implements OnInit {

  private alive=true;

  public TeamCL1_5: DeviceXpl;

  public velocidadCL1_5: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_5')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_5;
  ...
  changeCL1_5() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_5')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_5 = this.TeamCL1_5?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_5();
    });
  }
  ...
  dataCL1_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_5'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_5');

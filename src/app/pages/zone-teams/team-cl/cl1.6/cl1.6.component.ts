...
export interface CL1_6 {
  CL1_6_VelocidadActualMotor: number,
  CL1_6_CorrienteActualMotor: number,
  CL1_6_PotenciaActualMotor: number,
  CL1_6_TorqueActualMotor: number,
  CL1_6_KWh: number,
  CL1_6_voltaje: number
}
...
export class Cl1_6Component implements OnInit {

  private alive=true;

  public TeamCL1_6: DeviceXpl;

  public velocidadCL1_6: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_6')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_6;
  ...
  changeCL1_6() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_6')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_6 = this.TeamCL1_6?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_6();
    });
  }
  ...
  dataCL1_6(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_6'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_6');

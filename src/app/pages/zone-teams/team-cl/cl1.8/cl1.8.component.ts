...
export interface CL1_8 {
  CL1_8_VelocidadActualMotor: number,
  CL1_8_CorrienteActualMotor: number,
  CL1_8_PotenciaActualMotor: number,
  CL1_8_TorqueActualMotor: number,
  CL1_8_KWh: number,
  CL1_8_voltaje: number
}
...
export class Cl1_8Component implements OnInit {

  private alive=true;

  public TeamCL1_8: DeviceXpl;

  public velocidadCL1_8: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCL1_8')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCL1_8;
  ...
  changeCL1_8() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_8')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadCL1_8 = this.TeamCL1_8?.VelocidadActualMotor ?? 0;
    }
    this.dataCL1_8();
    });
  }
  ...
  dataCL1_8(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CL1_8'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CL1_8');

...
export interface MU1_2 {
  MU1_2_VelocidadActualMotor: number,
  MU1_2_CorrienteActualMotor: number,
  MU1_2_PotenciaActualMotor: number,
  MU1_2_TorqueActualMotor: number,
  MU1_2_KWh: number,
  MU1_2_voltaje: number
}
...
export class Mu1_2Component implements OnInit {

  private alive=true;

  public TeamMU1_2: DeviceXpl;

  public velocidadMU1_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeMU1_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadMU1_2;
  ...
  changeMU1_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamMU1_2 = res[0]
      this.velocidadMU1_2 = this.TeamMU1_2?.VelocidadActualMotor ?? 0;
    }
    this.dataMU1_2();
    });
  }
  ...
  dataMU1_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_2'))
    )
    .subscribe((res: any) => {

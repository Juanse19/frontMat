...
export interface MU1_3 {
  MU1_3_VelocidadActualMotor: number,
  MU1_3_CorrienteActualMotor: number,
  MU1_3_PotenciaActualMotor: number,
  MU1_3_TorqueActualMotor: number,
  MU1_3_KWh: number,
  MU1_3_voltaje: number
}
...
export class Mu1_3Component implements OnInit {

  private alive=true;

  public TeamMU1_3: DeviceXpl;

  public velocidadMU1_3: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeMU1_3')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadMU1_3;
  ...
  changeMU1_3() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_3')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadMU1_3 = this.TeamMU1_3?.VelocidadActualMotor ?? 0;
    }
    this.dataMU1_3();
    });
  }
  ...
  dataMU1_3(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_3'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data MU1_3');

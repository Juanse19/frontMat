...
export interface MU1_5M01 {
  MU1_5M01_VelocidadActualMotor: number,
  MU1_5M01_CorrienteActualMotor: number,
  MU1_5M01_PotenciaActualMotor: number,
  MU1_5M01_TorqueActualMotor: number,
  MU1_5M01_KWh: number,
  MU1_5M01_voltaje: number
}
...
export class Mu1_5Component implements OnInit {

  private alive=true;

  public TeamMU1_5M01: DeviceXpl;

  public velocidadMU1_5M01: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeMU1_5M01')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadMU1_5M01;
  ...
  changeMU1_5M01() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_5M01')
  ...
  dataMU1_5M01(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'MU1_5M01'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data MU1_5M01');

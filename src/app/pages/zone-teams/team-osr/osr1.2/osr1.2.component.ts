...
export interface osr1_2 {
  OSR1_2_VelocidadActualMotor: number,
  OSR1_2_CorrienteActualMotor: number,
  OSR1_2_PotenciaActualMotor: number,
  OSR1_2_TorqueActualMotor: number,
  OSR1_2_KWh: number,
  OSR1_2_voltaje: number
}
...
export class Osr1_2Component implements OnInit {

  private alive=true;

  public TeamOSR1_2: DeviceXpl;

  public velocidadOSR1_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeOs1_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadOSR1_2;
  ...
  changeOSR1_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_2')
  ...
  dataOSR1_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_2'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data OSR1_2');

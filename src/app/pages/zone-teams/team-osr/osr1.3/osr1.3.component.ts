...
public tooltipInterval1: number;
  ...
  changeOSR1_3() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_3')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadOSR1_3 = this.TeamOSR1_3?.VelocidadActualMotor ?? 0;
  ...
  dataOSR1_3(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_3'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data OSR1_3');

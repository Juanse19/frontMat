...
export interface osr1_4 {
  OSR1_4_VelocidadActualMotor: number,
  OSR1_4_CorrienteActualMotor: number,
  OSR1_4_PotenciaActualMotor: number,
  OSR1_4_TorqueActualMotor: number,
  OSR1_4_KWh: number,
  OSR1_4_voltaje: number
}
...
export class Osr1_4Component implements OnInit {

  private alive=true;

  public TeamOSR1_4: DeviceXpl;

  public velocidadOSR1_4: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeOs1_4')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadOSR1_4;
  ...
  changeOSR1_4() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_4')
  ...
  dataOSR1_4(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_4'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data OSR1_4');

...
export interface osr1_5 {
  OSR1_5_VelocidadActualMotor: number,
  OSR1_5_CorrienteActualMotor: number,
  OSR1_5_PotenciaActualMotor: number,
  OSR1_5_TorqueActualMotor: number,
  OSR1_5_KWh: number,
  OSR1_5_voltaje: number
}
...
export class Osr1_5Component implements OnInit {

  private alive=true;

  public TeamOSR1_5: DeviceXpl;

  public velocidadOSR1_5: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeOs1_5')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadOSR1_5;
  ...
  changeOSR1_5() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_5')
  ...
  dataOSR1_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_5'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data OSR1_5');

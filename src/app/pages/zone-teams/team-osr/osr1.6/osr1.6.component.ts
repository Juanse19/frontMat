...
export interface osr1_6 {
  OSR1_6M01_VelocidadActualMotor: number,
  OSR1_6M01_CorrienteActualMotor: number,
  OSR1_6M01_PotenciaActualMotor: number,
  OSR1_6M01_TorqueActualMotor: number,
  OSR1_6M01_KWh: number,
  OSR1_6M01_voltaje: number
}
...
export class Osr1_6Component implements OnInit {

  private alive=true;

  public TeamOSR1_6: DeviceXpl;

  public velocidadOSR1_6: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeOs1_6')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadOSR1_6;
  ...
  changeOSR1_6() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_6M01')
  ...
  dataOSR1_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'OSR1_6M01'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data OSR1_6');

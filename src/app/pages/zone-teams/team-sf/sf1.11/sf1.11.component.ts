...
export interface SF1_11 {
  SF1_11_VelocidadActualMotor: number,
  SF1_11_CorrienteActualMotor: number,
  SF1_11_PotenciaActualMotor: number,
  SF1_11_TorqueActualMotor: number,
  SF1_11_KWh: number,
  SF1_11_voltaje: number
}
...
export class Sf1_11Component implements OnInit {

  private alive=true;

  public TeamSF1_11: DeviceXpl;

  public velocidadSF1_11: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_11')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_11;
  ...
  changeSF1_11() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_11')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_11 = this.TeamSF1_11?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_11();
    });
  }
  ...
  dataSF1_11(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_11'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_11');

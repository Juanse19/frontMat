...
export interface SF1_12 {
  SF1_12_VelocidadActualMotor: number,
  SF1_12_CorrienteActualMotor: number,
  SF1_12_PotenciaActualMotor: number,
  SF1_12_TorqueActualMotor: number,
  SF1_12_KWh: number,
  SF1_12_voltaje: number
}
...
export class Sf1_12Component implements OnInit {

  private alive=true;

  public TeamSF1_12: DeviceXpl;

  public velocidadSF1_12: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_12')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_12;
  ...
  changeSF1_12() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_12')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_12 = this.TeamSF1_12?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_12();
    });
  }
  ...
  dataSF1_12(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_12'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_12');

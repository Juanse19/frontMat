...
export interface SF1_4 {
  SF1_4_VelocidadActualMotor: number,
  SF1_4_CorrienteActualMotor: number,
  SF1_4_PotenciaActualMotor: number,
  SF1_4_TorqueActualMotor: number,
  SF1_4_KWh: number,
  SF1_4_voltaje: number
}
...
export class Sf1_4Component implements OnInit {

  private alive=true;

  public TeamSF1_4: DeviceXpl;

  public velocidadSF1_4: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSf1_4')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_4;
  ...
  changeSF1_4() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_4')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_4 = this.TeamSF1_4?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_4();
    });
  }
  ...
  dataSF1_4(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_4'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_4');

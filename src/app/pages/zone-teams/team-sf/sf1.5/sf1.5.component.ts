...
export interface SF1_5 {
  SF1_5_VelocidadActualMotor: number,
  SF1_5_CorrienteActualMotor: number,
  SF1_5_PotenciaActualMotor: number,
  SF1_5_TorqueActualMotor: number,
  SF1_5_KWh: number,
  SF1_5_voltaje: number
}
...
export class Sf1_5Component implements OnInit {

  private alive=true;

  public TeamSF1_5: DeviceXpl;

  public velocidadSF1_5: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_5')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_5;
  ...
  changeSF1_5() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_5')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_5 = this.TeamSF1_5?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_5();
    });
  }
  ...
  dataSF1_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_5'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_5');

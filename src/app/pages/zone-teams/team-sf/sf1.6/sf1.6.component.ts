...
export interface SF1_6 {
  SF1_6_VelocidadActualMotor: number,
  SF1_6_CorrienteActualMotor: number,
  SF1_6_PotenciaActualMotor: number,
  SF1_6_TorqueActualMotor: number,
  SF1_6_KWh: number,
  SF1_6_voltaje: number
}
...
export class Sf1_6Component implements OnInit {

  private alive=true;

  public TeamSF1_6: DeviceXpl;

  public velocidadSF1_6: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_6')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_6;
  ...
  changeSF1_6() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_6')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_6 = this.TeamSF1_6?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_6();
    });
  }
  ...
  dataSF1_6(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_6'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_6');

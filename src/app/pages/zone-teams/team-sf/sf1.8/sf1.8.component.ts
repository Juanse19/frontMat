...
export interface SF1_8 {
  SF1_8_VelocidadActualMotor: number,
  SF1_8_CorrienteActualMotor: number,
  SF1_8_PotenciaActualMotor: number,
  SF1_8_TorqueActualMotor: number,
  SF1_8_KWh: number,
  SF1_8_voltaje: number
}
...
export class Sf1_8Component implements OnInit {

  private alive=true;

  public TeamSF1_8: DeviceXpl;

  public velocidadSF1_8: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_8')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_8;
  ...
  changeSF1_8() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_8')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_8 = this.TeamSF1_8?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_8();
    });
  }
  ...
  dataSF1_8(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_8'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_8');

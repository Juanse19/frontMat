...
export interface SF3_10 {
  SF3_10_VelocidadActualMotor: number,
  SF3_10_CorrienteActualMotor: number,
  SF3_10_PotenciaActualMotor: number,
  SF3_10_TorqueActualMotor: number,
  SF3_10_KWh: number,
  SF3_10_voltaje: number
}
...
export class Sf3_10Component implements OnInit {

  private alive=true;

  public TeamSF3_10: DeviceXpl;

  public velocidadSF3_10: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_10')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_10;
  ...
  changeSF3_10() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_10')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_10 = this.TeamSF3_10?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_10();
    });
  }
  ...
  dataSF3_10(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_10'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_10');

...
export interface SF3_5 {
  SF3_5_VelocidadActualMotor: number,
  SF3_5_CorrienteActualMotor: number,
  SF3_5_PotenciaActualMotor: number,
  SF3_5_TorqueActualMotor: number,
  SF3_5_KWh: number,
  SF3_5_voltaje: number
}
...
export class Sf3_5Component implements OnInit {

  private alive=true;

  public TeamSF3_5: DeviceXpl;

  public velocidadSF3_5: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_5')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_5;
  ...
  changeSF3_5() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_5')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_5 = this.TeamSF3_5?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_5();
    });
  }
  ...
  dataSF3_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_5'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_5');

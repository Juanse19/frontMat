...
export interface SF3_6 {
  SF3_6_VelocidadActualMotor: number,
  SF3_6_CorrienteActualMotor: number,
  SF3_6_PotenciaActualMotor: number,
  SF3_6_TorqueActualMotor: number,
  SF3_6_KWh: number,
  SF3_6_voltaje: number
}
...
export class Sf3_6Component implements OnInit {

  private alive=true;

  public TeamSF3_6: DeviceXpl;

  public velocidadSF3_6: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_6')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_6;
  ...
  changeSF3_6() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_6')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_6 = this.TeamSF3_6?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_6();
    });
  }
  ...
  dataSF3_6(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_6'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_6');

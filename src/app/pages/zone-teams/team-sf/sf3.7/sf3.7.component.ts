...
export interface SF3_7 {
  SF3_7_VelocidadActualMotor: number,
  SF3_7_CorrienteActualMotor: number,
  SF3_7_PotenciaActualMotor: number,
  SF3_7_TorqueActualMotor: number,
  SF3_7_KWh: number,
  SF3_7_voltaje: number
}
...
export class Sf3_7Component implements OnInit {

  private alive=true;

  public TeamSF3_7: DeviceXpl;

  public velocidadSF3_7: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_7')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_7;
  ...
  changeSF3_7() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_7')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_7 = this.TeamSF3_7?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_7();
    });
  }
  ...
  dataSF3_7(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_7'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_7');

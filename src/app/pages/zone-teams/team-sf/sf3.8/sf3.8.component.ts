...
export interface SF3_8 {
  SF3_8_VelocidadActualMotor: number,
  SF3_8_CorrienteActualMotor: number,
  SF3_8_PotenciaActualMotor: number,
  SF3_8_TorqueActualMotor: number,
  SF3_8_KWh: number,
  SF3_8_voltaje: number
}
...
export class Sf3_8Component implements OnInit {

  private alive=true;

  public TeamSF3_8: DeviceXpl;

  public velocidadSF3_8: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_8')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_8;
  ...
  changeSF3_8() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_8')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_8 = this.TeamSF3_8?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_8();
    });
  }
  ...
  dataSF3_8(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_8'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_8');

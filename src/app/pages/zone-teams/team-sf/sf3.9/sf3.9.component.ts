...
export interface SF3_9 {
  SF3_9_VelocidadActualMotor: number,
  SF3_9_CorrienteActualMotor: number,
  SF3_9_PotenciaActualMotor: number,
  SF3_9_TorqueActualMotor: number,
  SF3_9_KWh: number,
  SF3_9_voltaje: number
}
...
export class Sf3_9Component implements OnInit {

  private alive=true;

  public TeamSF3_9: DeviceXpl;

  public velocidadSF3_9: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_9')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_9;
  ...
  changeSF3_9() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_9')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_9 = this.TeamSF3_9?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_9();
    });
  }
  ...
  dataSF3_9(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_9'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF3_9');

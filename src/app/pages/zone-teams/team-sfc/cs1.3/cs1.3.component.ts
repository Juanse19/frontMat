...
export class Cs1_3Component implements OnInit {

  private alive=true;

  public TeamCS1_3M01: DeviceXpl;

  public velocidadCS1_3M01: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeCS1_3M01')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadCS1_3M01;
  ...
  changeCS1_3M01() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CS1_3')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamCS1_3M01 = res[0]
      // this.dataSF1_1 = TeamSF1_1
      this.velocidadCS1_3M01 = this.decimalPipe.transform(this.TeamCS1_3M01?.VelocidadActualMotor) ?? 0;
    }
    });
  }
  ...
  dataCS1_3M01(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'CS1_3'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data CS1_3M01');

...
export class Sf1_2Component implements OnInit {

  private alive=true;

  public TeamSF1_2: DeviceXpl;

  public velocidadSF1_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF1_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF1_2;
  ...
  changeSF1_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF1_2 = this.TeamSF1_2?.VelocidadActualMotor ?? 0;
    }
    this.dataSF1_2()
    });
  }
  ...
  dataSF1_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF1_2'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            console.log('no hay data SF1_2');

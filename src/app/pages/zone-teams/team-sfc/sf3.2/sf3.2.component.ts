...
export interface SF3_2 {
  SF3_2_VelocidadActualMotor: number,
...
export class Sf3_2Component implements OnInit {

  private alive=true;

  public TeamSF3_2: DeviceXpl;

  public velocidadSF3_2: any = 0;
  intervalSubscriptionItems?: Subscription;

  @ViewChild('gaugeSF3_2')
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSF3_2;
  ...
  changeSF3_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSF3_2 = this.TeamSF3_2?.VelocidadActualMotor ?? 0;
    }
    this.dataSF3_2();
    });
  }
  ...
  dataSF3_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/device?device=' + 'SF3_2'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
            // console.log('no hay data SF3_2');

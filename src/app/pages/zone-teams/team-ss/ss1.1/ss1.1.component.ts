...
export interface ss1_1 {
  SS1_1_VelocidadActualMotor: number,
  SS1_1_CorrienteActualMotor: number,
  SS1_1_PotenciaActualMotor: number,
  SS1_1_TorqueActualMotor: number,
  SS1_1_KWh: number,
  SS1_1_voltaje: number
}
...
export class Ss1_1Component implements OnInit {
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSS1_1;
...
changeRange() {
  ...
  dataSS1_1(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.apiUrlNode1 + '/api/v1/device?device=' + 'SS1_1'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
        console.log('No hay data SS1_1');

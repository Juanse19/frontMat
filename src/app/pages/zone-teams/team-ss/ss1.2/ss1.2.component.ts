...
export interface ss1_2 {
  SS1_2_VelocidadActualMotor: number,
  SS1_2_CorrienteActualMotor: number,
  SS1_2_PotenciaActualMotor: number,
  SS1_2_TorqueActualMotor: number,
  SS1_2_KWh: number,
  SS1_2_voltaje: number
}
...
export class Ss1_2Component implements OnInit {

  private alive=true;

  public TeamSS1_2: DeviceXpl;

  public velocidadSS1_2: any = 0;
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSS1_2;
  ...
  changeSS1_2() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_2')
    ...
    .subscribe((res: any)=>{
      ...
      this.TeamSS1_2 = res[0]
      this.velocidadSS1_2 = this.TeamSS1_2?.VelocidadActualMotor ?? 0;
    }
    this.dataSS1_2();
    });
  }
  ...
  dataSS1_2(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_2'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
        console.log('No hay data SS1_2');

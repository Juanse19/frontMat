...
export interface ss1_3 {
  SS1_3_VelocidadActualMotor: number,
  SS1_3_CorrienteActualMotor: number,
  SS1_3_PotenciaActualMotor: number,
  SS1_3_TorqueActualMotor: number,
  SS1_3_KWh: number,
  SS1_3_voltaje: number
}
...
export class Ss1_3Component implements OnInit {

  private alive=true;

  public TeamSS1_3: DeviceXpl;

  public velocidadSS1_3: any = 0;
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSS1_3;
  ...
  changeSS1_3() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_3')
    ...
    .subscribe((res: any)=>{
      ...
      this.velocidadSS1_3 = this.TeamSS1_3?.VelocidadActualMotor ?? 0;
  ...
  dataSS1_3(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_3'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
        console.log('No hay data SS1_3');

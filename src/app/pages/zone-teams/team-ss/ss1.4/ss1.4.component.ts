...
export interface ss1_4 {
  SS1_4_VelocidadActualMotor: number,
  SS1_4_CorrienteActualMotor: number,
  SS1_4_PotenciaActualMotor: number,
  SS1_4_TorqueActualMotor: number,
  SS1_4_KWh: number,
  SS1_4_voltaje: number
}
...
export class Ss1_4Component implements OnInit {

  private alive=true;

  public TeamSS1_4: DeviceXpl;

  public velocidadSS1_4: any = 0;
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSS1_4;
  ...
  changeSS1_4() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_4')
  ...
  dataSS1_4(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_4'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
        console.log('No hay data SS1_4');

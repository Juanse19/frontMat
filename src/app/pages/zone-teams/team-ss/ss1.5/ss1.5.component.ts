...
export interface ss1_5 {
  SS1_5_VelocidadActualMotor: number,
  SS1_5_CorrienteActualMotor: number,
...
export class Ss1_5Component implements OnInit {

  private alive=true;

  public TeamSS1_5: DeviceXpl;
...
public tooltipInterval1: number;
  ...
  gauge(){
    if (document.getElementById('sample-data')) {
      let value1: number = this.velocidadSS1_5;
  ...
  changeSS1_5() {
    this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_5')
  ...
  dataSS1_5(){
    ...
    .pipe(
      ...
      switchMap( () => this.apiGetComp.GetJson(this.api.urlapixplmatbagapp02 + '/api/v1/getdevice?device=' + 'SS1_5'))
    )
    .subscribe((res: any) => {
      if (JSON.stringify(res)=='{}') {
        console.log('No hay data SS1_5');
